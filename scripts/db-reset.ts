...
const sqlDrop = `
DROP TABLE IF EXISTS "users", "accounts", "sessions", "verification_tokens", "media_assets", "media_locations", "content_blocks", "blog_posts", "audit_logs", "settings" CASCADE;
DROP TYPE IF EXISTS "Role", "MediaStatus", "MediaType", "BlogStatus" CASCADE;
`;
...
async function reset() {
  console.log("🔥 Dropping conflicting tables via WebSocket...");
  const dropCommands = sqlDrop
    .split(";")
    .map((cmd) => cmd.trim())
  ...
  console.log("🚀 Creating fresh tables via WebSocket...");
  const createCommands = sqlCreate
    .split(";")
    .map((cmd) => cmd.trim())
    .filter((cmd) => cmd.length > 0);
  ...
  for (const cmd of createCommands) {
    ...
    console.log(`Executing CREATE statement: ${firstLine}...`);
    await prisma.$executeRawUnsafe(cmd);
  }
